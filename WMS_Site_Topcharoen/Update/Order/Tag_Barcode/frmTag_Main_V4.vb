...
#End Region
    ...
    Private Sub frmTag_Main_V2_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Me.grdTagAll.AutoGenerateColumns = False
            Me.cboQtyPerTagAuto.SelectedIndex = 0
            Me.cboQtyPerTagManual.SelectedIndex = 2
            Me.getReciveType()
            Me.getProductType()
            ...
            'SET NEW GUID OPEN FORM
            Me._GuID = Guid.NewGuid.ToString()
...
#End Region
    ...
    Private Sub getAllTAG(ByVal StrSQL As String)
        Dim objTAG As New tb_TAG(tb_TAG.enuOperation_Type.SEARCH)
        Dim objDT As DataTable = New DataTable
        Dim strWhere As String = ""
        'strWhere &= " And Order_Index = '" & _Order_Index & "'"
        strWhere &= " AND Tag_Process_Id = 1 "
        strWhere &= " AND GuID NOT IN ('" + _GuID + "') "
        strWhere &= StrSQL
        objTAG.getView_Tag_Header(strWhere)
        objDT = objTAG.DataTable

        Me.grdTagAll.Refresh()
        grdTagAll.DataSource = objDT
        Dim Irow As Integer = 1
        For Each dr As DataRow In objDT.Rows
            dr("Row") = Irow
            Irow = Irow + 1
        ...
        For i As Integer = 0 To Me.grdTagAll.Rows.Count - 1
            Select Case Me.grdTagAll.Rows(i).Cells("Col_All_TAG_Status_Index").Value
                Case "2"
                    Me.grdTagAll.Rows(i).Cells("col_All_Status").Style.BackColor = Color.LightGreen
                    Me.grdTagAll.Rows(i).Cells("col_All_TAG_NO").Style.BackColor = Color.LightGreen
                Case Else
                    Select Case Me.grdTagAll.Rows(i).Cells("Col_All_Print_Status").Value
                        Case 1
                            Me.grdTagAll.Rows(i).Cells("col_All_Status").Style.BackColor = Color.LightSkyBlue
                            Me.grdTagAll.Rows(i).Cells("col_All_TAG_NO").Style.BackColor = Color.LightSkyBlue
                        Case Else
                            Me.grdTagAll.Rows(i).Cells("col_All_Status").Style.BackColor = Color.White
                            Me.grdTagAll.Rows(i).Cells("col_All_TAG_NO").Style.BackColor = Color.White
                    End Select
    ...
    Private Function StringSearch() As String
        Dim StrSQL As String = ""
        If Me.chkSKU.Checked = True And Me._SKU_Index <> "" Then
            StrSQL &= " AND SKU_Index = '" & Me._SKU_Index & "'"
        End If

        If Me.chkProductType.Checked = True And cbProductType.SelectedValue <> "" Then
            StrSQL &= " AND ProductType_Index = '" & cbProductType.SelectedValue & "'"
        End If

        If Me.chkReciveType.Checked = True And cbReciveType.SelectedValue <> 0 Then
            StrSQL &= " AND DocumentType_Index = '" & cbReciveType.SelectedValue & "'"
        End If

        If Me.chkRef_No1.Checked = True And txtRef_No1.Text.Trim <> "" Then
            StrSQL &= " AND Ref_No1 = '" & Me.txtRef_No1.Text.Trim & "'"
        End If
    ...
    Private Sub btnSearch_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        Dim StrSQL As String = " AND (Qty > 0) "
        Try
            ...
            If drGenTag.Length > 0 Then
                ...
                If RowError = -1 Then ' Check Validate -1 = Okay
                    Select Case Me.cboQtyPerTagManual.SelectedIndex
                        ...
                        Case 2
                            Dim strOrderItemIndex As String = ""
                            drGenTag = CType(Me.grdOrderItem.DataSource, DataTable).Select("chkSelect = 1")
                            If drGenTag.Length = 0 Then
                                W_MSG_Information_ByIndex("300032")
                                Exit Sub
                            End If
                            For Each DrOrderItem As DataRow In drGenTag
                                strOrderItemIndex &= " '" + DrOrderItem("OrderItem_Index") + "' ,"
                            Next

                            If strOrderItemIndex.Length > 0 Then strOrderItemIndex = strOrderItemIndex.Substring(0, strOrderItemIndex.Length - 1)

                            Dim frm As New frmTag_Add_M_to_One("", "", strOrderItemIndex)
                            frm.GuID = _GuID
                            frm.ShowDialog()
                    ...
                    Select Case Me.cboQtyPerTagAuto.SelectedIndex
                        Case 0 'Qty Per Pallet 
                            ...
                            For Each DrOrderItem As DataRow In drGenTag
                                ...
                                While 0 < tmpQty
                                    Dim objItemPerQty As New tb_TAG_Update(tb_TAG_Update.enuOperation_Type.ADDNEW)
    ...
    Private Sub btnCancel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCancel.Click
        Try
            If tcTagList.SelectedIndex = 0 Then
                ...
                If W_MSG_Confirm_ByIndex(5) = Windows.Forms.DialogResult.Yes Then
                    ...
                    Next
                    getTAG(StringSearch())
                    getAllTAG(StringSearch())
                    btnSearch_Click_1(sender, e)
                End If
            ElseIf tcTagList.SelectedIndex = 1 Then
                If Me.grdTagAll.Rows.Count = 0 Then
                    W_MSG_Error_ByIndex(76)
                    Exit Sub
                End If
                ...
                If Me.grdTagAll.Rows.Count <= 0 Then Exit Sub
                If W_MSG_Confirm_ByIndex(5) = Windows.Forms.DialogResult.Yes Then
                    Dim dtDelTag As New DataTable
                    dtDelTag = CType(grdTagAll.DataSource, DataTable)
                    dtDelTag.AcceptChanges()
                    Dim drDelTagArr() As DataRow = dtDelTag.Select("chkSelect=1 and TAG_Status = 1")
                    Dim objTag As New tb_TAG(tb_TAG.enuOperation_Type.DELETE)
                    For Each drDelTag As DataRow In drDelTagArr
    ...
    Private Sub ChkAllTagAll_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ChkAllTagAll.CheckedChanged
        Try
            If Me.grdTagAll.Rows.Count > 0 Then
                Dim Dttmp As New DataTable
                Dttmp = CType(Me.grdTagAll.DataSource, DataTable)
                If Me.ChkAllTagAll.Checked = True Then
                    For Each Dr As DataRow In Dttmp.Rows
                        Dr("chkSelect") = 1
                    Next
                Else
    ...
    Private Sub grdTagAll_CellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdTagAll.CellClick
        Try
            Select Case CType(sender, DataGridView).CurrentCell.OwningColumn.Name
                Case "Col_All_BtnAdd"
                    Dim frm As New WMS_STD_INB_Receive.frmPopUpTagDuplicate(Me.grdTag.Rows(Me.grdTag.CurrentRow.Index).Cells("col_TAG_NO").Value.ToString, _Order_Index)
    ...
    Private Sub btnSaveTag_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSaveTag.Click
        Try
            ...
            If tcTagList.SelectedIndex = 0 Then
                ...
                Dim _IsTemporaryTAG As Boolean = False
                For Each dr As DataRow In drGenTag
                    ListTag_Index.Add(dr("TAG_Index"))
                    If dr("IsTemporaryTAG") = True Then
                        _IsTemporaryTAG = True
                    End If
                Next

                If _IsTemporaryTAG Then
                    W_MSG_Information("ไม่สามารถจัดเก็บ TAG Temporary ได้")
                    Me.btnSearch_Click_1(sender, e)
                Else
            ...
            ElseIf tcTagList.SelectedIndex = 1 Then
                Dim drGenTag() As DataRow
                Dim ListTag_Index As New List(Of String)
                CType(Me.grdTagAll.DataSource, DataTable).AcceptChanges()
                drGenTag = CType(Me.grdTagAll.DataSource, DataTable).Select("chkSelect = 1 AND TAG_Status = 1 ")
                'drGenTag = CType(Me.grdTag.DataSource, DataTable).Select("TAG_Status = 1 ")
                Dim _IsTemporaryTAG As Boolean = False
                For Each dr As DataRow In drGenTag
                    ListTag_Index.Add(dr("TAG_Index"))
                ...
                Else
                    If ListTag_Index.Count > 0 Then
                        Dim frm As New WMS_STD_INB_Receive.frmPutawayWithTAG
                        'Dim frm As New frmPutawayWithTAG
                        frm.Order_Index = grdTagAll.Rows(grdTagAll.CurrentRow.Index).Cells("Col_All_Order_Index").Value.ToString
                        frm.TAG_Index = ListTag_Index
                        frm.ShowDialog()
                        btnSearch_Click_1(sender, e)
                    Else
                ...
                ElseIf tcTagList.SelectedIndex = 1 Then
                    Select Case Report_Name.ToUpper
                        Case "TAGSTICKERPRINTOUT", "TAGSTICKERPRINTOUT_MINI", "TAGSTICKERCHEMICALPRINTOUT_MINI", "TAGSTICKERSPBRPRINTOUT_MINI"
                            Dim oReport As New clsReport()
                            Dim TAG_Index_IN As String = ""
                            Dim drArr() As DataRow = DirectCast(Me.grdTagAll.DataSource, DataTable).Select(" chkSelect=1 and TAG_Status <> -1 ")
                            If drArr.Length > 0 Then
                                For Each dr As DataRow In drArr
                                    TAG_Index_IN &= "'" & dr("TAG_Index").ToString() & "',"
    ...
    Private Sub tcTagList_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tcTagList.SelectedIndexChanged
        Try
            Select Case tcTagList.SelectedIndex
                Case 0
                    If grdTag.Rows.Count > 0 Then
                        Me.lbCountRowsTag.Text = "จำนวน " & grdTag.Rows.Count.ToString & " รายการ"
                    Else
                        Me.lbCountRowsTag.Text = "ไม่พบรายการ"
                    End If
                Case 1
                    If grdTagAll.Rows.Count > 0 Then
                        Me.lbCountRowsTag.Text = "จำนวน " & grdTagAll.Rows.Count.ToString & " รายการ"
                    Else
                        Me.lbCountRowsTag.Text = "ไม่พบรายการ"
                    End If
                Case Else
