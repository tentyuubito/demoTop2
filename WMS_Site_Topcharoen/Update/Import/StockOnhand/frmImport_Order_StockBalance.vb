...
Public Class frmImport_Order_StockBalance
    ...
    Public Function LoadDataFromFile(ByVal pstrFileName As String, ByVal pstrWorkSheet As String) As DataTable
        Try
            ...
            Dim obj As New bl_ImportOrder_Stock_TNP
            Dim objDT As New DataTable
            objDT = obj.LoadWorkSheet(pstrFileName)

            If objDT.Rows.Count > 0 Then
                If pstrWorkSheet.Trim = "" Then
                    pstrWorkSheet = objDT.Rows(0).Item("TABLE_NAME").ToString.Replace("$", "")
                    Me.txtWorkSheet.Text = pstrWorkSheet
                End If
            End If
            ...
            Dim IMPORTSTOCK_DOCUMENT_TYPE_INDEX As String = AppSettings("IMPORTSTOCK_DOCUMENT_TYPE_INDEX")

            With odaSource
                strSQL = String.Format(" SELECT '' AS Check_Data,'' as Customer_Index,'' as ItemStatus_Index ,'{0}' as DocumentType_Index,'' as Package_Index,'' as Sku_Index ", IMPORTSTOCK_DOCUMENT_TYPE_INDEX)
                strSQL &= " ,'' as Supplier_Index,0 as Line_No,*"
                strSQL &= " FROM [" & pstrWorkSheet & "$] "
                strSQL &= " WHERE SKU <> ''  and QTY > 0 "
                strSQL &= " ORDER BY RECEIVE_DATE asc,RECEIVE_NO asc,SKU asc,LOT asc"
        ...
        Try
            ...
            For i As Integer = 0 To Me.grdPreviewData.RowCount - 1
                ...
                With grdPreviewData.Rows(i)
                    ...
                    'Sku and Package
                    If .Cells("SKU").Value = "FP000065" Then
                        .Cells("SKU").Value = "FP000065"
                    End If
                    tempDataSku = IsExitDataSku_ByCustomer(.Cells("SKU").Value, Me.txtCustomer_Id.Tag)
                    If tempDataSku.Rows.Count = 0 Then
                        ...
                        Continue For
                    Else
                        Dim pk As String = GetPackage_Sku(tempDataSku.Rows(0).Item("Sku_Index").ToString, .Cells("UOM").Value)

                        .Cells("Sku_Index").Value = tempDataSku.Rows(0).Item("Sku_Index").ToString
                        .Cells("Package_Index").Value = pk 'tempDataSku.Rows(0).Item("Package_Index").ToString

                        If pk = "" Then
                            .Cells("Check_Data").Value = "ไม่พบข้อมูลหน่วย SKU นี้ ! ของลูกค้า : " & Me.txtCustomer_Name.Text
                            .Cells("Check_Data").Style.BackColor = Color.Red
                            strResult = .Cells("Check_Data").Value
                            Continue For
                        End If
                    ...
                    If IsDate(.Cells("MFG_DATE").Value) = False And IsDate(.Cells("EXP_DATE").Value) = False Then
                        .Cells("Check_Data").Value = "กรุณาตรวจสอบวันที่ผลิต/หมดอายุ"
                        .Cells("Check_Data").Style.BackColor = Color.Red
                        strResult = .Cells("Check_Data").Value
                        Continue For
                    Else
                        Dim expY As Double = 0
                        Dim expM As Double = 0
                        Dim expD As Double = 0
                        expY = IIf(IsNumeric(tempDataSku.Rows(0)("ItemLife_y")), tempDataSku.Rows(0)("ItemLife_y"), 0)
                        expM = IIf(IsNumeric(tempDataSku.Rows(0)("ItemLife_m")), tempDataSku.Rows(0)("ItemLife_m"), 0)
                        expD = IIf(IsNumeric(tempDataSku.Rows(0)("ItemLife_d")), tempDataSku.Rows(0)("ItemLife_d"), 0)
                        Try
                            If IsDate(.Cells("MFG_DATE").Value) = True And IsDate(.Cells("EXP_DATE").Value) = False Then
                                Dim tmpDate As Date = .Cells("MFG_DATE").Value
                                tmpDate = tmpDate.AddYears(expY)
                                tmpDate = tmpDate.AddMonths(expM)
                                tmpDate = tmpDate.AddDays(expD)
                                .Cells("EXP_DATE").Value = tmpDate
                                .Cells("EXP_DATE").Style.BackColor = Color.YellowGreen
                            ElseIf IsDate(.Cells("MFG_DATE").Value) = False And IsDate(.Cells("EXP_DATE").Value) = True Then
                                Dim tmpDate As Date = .Cells("EXP_DATE").Value
                                tmpDate = tmpDate.AddYears(-expY)
                                tmpDate = tmpDate.AddMonths(-expM)
                                tmpDate = tmpDate.AddDays(-expD)
                                .Cells("MFG_DATE").Value = tmpDate
                                .Cells("MFG_DATE").Style.BackColor = Color.YellowGreen
                            End If
                        Catch ex As Exception
                            .Cells("Check_Data").Value = "กรุณาตรวจสอบวันที่ผลิต/หมดอายุ"
                            .Cells("Check_Data").Style.BackColor = Color.Red
                    ...
                    If tempDataSku.Rows.Count > 0 Then
                        'WGT_UOM
                        If IsNumeric(.Cells("WGT_UOM").Value.ToString.Trim) = False Then
                            'UnitWeight_Index
                            .Cells("WGT_UOM").Value = IIf(IsNumeric(tempDataSku.Rows(0).Item("UnitWeight_Index")), tempDataSku.Rows(0).Item("UnitWeight_Index"), 0)
                            .Cells("WGT_UOM").Style.BackColor = Color.YellowGreen
                        End If
                        'WGT_KG
                        If IsNumeric(.Cells("WGT_KG").Value.ToString.Trim) = False Then
                            'UnitWeight_Index * QTY
                            .Cells("WGT_KG").Value = IIf(IsNumeric(.Cells("WGT_UOM").Value) And IsNumeric(.Cells("QTY").Value), (CDbl(.Cells("WGT_UOM").Value) * CDbl(.Cells("QTY").Value)).ToString("0.00"), 0)
                            .Cells("WGT_KG").Style.BackColor = Color.YellowGreen
                        End If
                        'VOL_UOM
                        If IsNumeric(.Cells("VOL_UOM").Value.ToString.Trim) = False Then
                            'Unit_Volume 
                            .Cells("VOL_UOM").Value = IIf(IsNumeric(tempDataSku.Rows(0).Item("Unit_Volume")), tempDataSku.Rows(0).Item("Unit_Volume"), 0)
                            .Cells("VOL_UOM").Style.BackColor = Color.YellowGreen
                        End If
                        'VOL_M3
                        If IsNumeric(.Cells("VOL_M3").Value.ToString.Trim) = False Then
                            'Unit_Volume * QTY
                            .Cells("VOL_M3").Value = IIf(IsNumeric(.Cells("VOL_UOM").Value) And IsNumeric(.Cells("QTY").Value), (CDbl(.Cells("VOL_UOM").Value) * CDbl(.Cells("QTY").Value)).ToString("0.00"), 0)
                            .Cells("VOL_M3").Style.BackColor = Color.YellowGreen
                        End If
                    End If
                    ...
                    'Location
                    strLocation = ValidateValueType.IsNullOrEmptyOrDBNullOrNothingGetData(.Cells("LOCATION_ALIAS").Value, GetType(String))
                    ...
                    'Order_Date
                    If IsDate(.Cells("RECEIVE_DATE").Value) = False Then
                        .Cells("Check_Data").Value = "กรุณาตรวจสอบวันที่รับ"
                        .Cells("Check_Data").Style.BackColor = Color.Red
                        strResult = .Cells("Check_Data").Value
                        Continue For
                    Else
                        .Cells("RECEIVE_DATE").Value = CDate(.Cells("RECEIVE_DATE").Value).Date.ToString("dd/MM/yyyy")
                    End If


                    'If .Cells("Exp_Date").Value.ToString = "" Then
                    '    Dim sql = ""
                    'End If

                    'Weight
                    If IsNumeric(.Cells("WGT_UOM").Value) = False Then
                        .Cells("Check_Data").Value = "กรุณาตรวจสอบ WGT_UOM"
                        '.Cells("Weight").Value = 0
                        'getSKU_Detail(.Cells("Sku_Id").Value, i, .Cells("Customer_Index").Value, "WGT_UOM")
                    End If

                    'Net Weight
                    If IsNumeric(.Cells("WGT_KG").Value) = False Then
                        .Cells("Check_Data").Value = "กรุณาตรวจสอบ WGT_KG"
                        '.Cells("Flo1").Value = 0
                        'getSKU_Detail(.Cells("Sku_Id").Value, i, .Cells("Customer_Index").Value, "NW")
                        '.Cells("Flo1").Value = .Cells("Weight").Value
                    End If

                    If IsNumeric(.Cells("VOL_UOM").Value) = False Then
                        .Cells("Check_Data").Value = "กรุณาตรวจสอบ VOL_UOM"
                        '.Cells("Flo1").Value = 0
                        'getSKU_Detail(.Cells("Sku_Id").Value, i, .Cells("Customer_Index").Value, "NW")
                        '.Cells("Flo1").Value = .Cells("Weight").Value
                    End If

                    If IsNumeric(.Cells("VOL_M3").Value) = False Then
                        .Cells("Check_Data").Value = "กรุณาตรวจสอบ VOL_M3"
                        '.Cells("Flo1").Value = 0
                        'getSKU_Detail(.Cells("Sku_Id").Value, i, .Cells("Customer_Index").Value, "NW")
                        '.Cells("Flo1").Value = .Cells("Weight").Value
                    End If
                    ...
                    'Plot
                    .Cells("LOT").Value = ValidateValueType.IsNullOrEmptyOrDBNullOrNothingGetData(.Cells("LOT").Value, GetType(String))
...
#Region "   PregressBar Imports  "
    Private Sub BackgroundWorker1_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles BwgImport.DoWork
        ...
        Try
            ...
            dtPrimaryKey.Columns.Add("RECEIVE_DATE", GetType(Date))
            dtPrimaryKey.Columns.Add("RECEIVE_NO", GetType(String))
            ...
            For Each odrOrder As DataRow In Me._dtDataSource.Rows
                Dim drArrRow() As DataRow = dtPrimaryKey.Select("RECEIVE_DATE='" & odrOrder("RECEIVE_DATE") & "' and RECEIVE_NO='" & odrOrder("RECEIVE_NO").ToString & "'")
                If drArrRow.Length = 0 Then
                    Dim drNewRow As DataRow
                    drNewRow = dtPrimaryKey.NewRow
                    drNewRow("RECEIVE_DATE") = odrOrder("RECEIVE_DATE")
                    drNewRow("RECEIVE_NO") = odrOrder("RECEIVE_NO")
            ...
            For Each drPrimarykey As DataRow In dtPrimaryKey.Rows
                ...
                Me._tmpRef_No3 = drPrimarykey("RECEIVE_NO").ToString
                Dim drArrOrder() As DataRow = Me._dtDataSource.Select("RECEIVE_DATE='" & CDate(drPrimarykey("RECEIVE_DATE")).ToString("dd/MM/yyyy") & "' and isnull(RECEIVE_NO,'') ='" & drPrimarykey("RECEIVE_NO").ToString & "'")
                ...
                For Each odrOrderItem As DataRow In drArrOrder
                    ...
                    If iRow = 0 Then
                        ...
                        objHeader.Order_No = odrOrderItem("RECEIVE_NO").ToString
                        objHeader.Order_Index = objSys_MaxID.getSys_Value("Order_Index")
                        strDeleteOrder_Index = objHeader.Order_Index  'Temp For delete
                        objHeader.Order_Date = CDate(odrOrderItem("RECEIVE_DATE").ToString).ToString("yyyy/MM/dd")
                        ...
                        objHeader.Ref_No1 = IIf(odrOrderItem("INVOICE") Is Nothing, "", odrOrderItem("INVOICE").ToString) 'odrOrderItem("Ref_No2").ToString '""
                    ...
                    dtTemp = IsExitDataSku_ByCustomer(odrOrderItem("SKU"), odrOrderItem("Customer_Index"))
                    'W
                    ...
                    'Weight_Per_Pck
                    objItem.Weight_Per_Pck = odrOrderItem("WGT_UOM") 'ValidateValueType.IsNullOrEmptyOrDBNullOrNothingGetData(dtTemp.Rows(0)("WGT_UOM"), GetType(Double))
                    'Volume_Per_Pck
                    objItem.Volume_Per_Pck = odrOrderItem("VOL_UOM") 'ValidateValueType.IsNullOrEmptyOrDBNullOrNothingGetData(dtTemp.Rows(0)("VOL_UOM"), GetType(Double))
                    ...
                    'OderItem_Price
                    objItem.OrderItem_Price = 0 'FormatNumber(odrOrderItem("Flo1"), 4) 'FormatNumber((objItem.Qty * objItem.Price_Per_Pck), 2)
                    'Price_Per_Pck
                    objItem.Price_Per_Pck = 0 'FormatNumber(odrOrderItem("Flo1") / objItem.Qty, 4)  'ValidateValueType.IsNullOrEmptyOrDBNullOrNothingGetData(dtTemp.Rows(0)("Price1"), GetType(Double))
                    ...
                    'Weight
                    objItem.Weight = FormatNumber(odrOrderItem("WGT_KG"), 4)
                    'col_Net_Weight
                    objItem.Flo1 = FormatNumber(odrOrderItem("WGT_KG"), 4) 'objItem.Flo1 = FormatNumber(odrOrderItem("Weight"), 4)
                    'Volume
                    objItem.Volume = FormatNumber(odrOrderItem("VOL_M3"), 4) 'FormatNumber((objItem.Qty * objItem.Volume_Per_Pck), 4) ' ไม่มีมาตั้งแต่แรก
                    'PLot
                    ...
                    If odrOrderItem("MFG_DATE").ToString <> "" Then
                        objItem.Mfg_Date = odrOrderItem("MFG_DATE") 'CDate(odrOrderItem("EXP_DATE").ToString).ToString("yyyy/MM/dd")
                        objItem.IsMfg_Date = True
                    Else
                        objItem.Mfg_Date = CDate(odrOrderItem("RECEIVE_DATE").ToString).ToString("yyyy/MM/dd")
                        objItem.IsMfg_Date = False
                    End If
                    objItem.IsExp_Date = False
                    If odrOrderItem("EXP_DATE").ToString <> "" Then
                        objItem.Exp_date = odrOrderItem("EXP_DATE") 'CDate(odrOrderItem("EXP_DATE").ToString).ToString("yyyy/MM/dd")
                    ...
                    objItem.Str4 = odrOrderItem("LOCATION_ALIAS").ToString 'Location
                    objItem.Str5 = odrOrderItem("PALLET_ID").ToString
                    objItem.Comment = "Import Data"
                    ' *** Add value ***
                    objItem.Str1 = IIf(odrOrderItem("ERP") Is Nothing, "", odrOrderItem("ERP").ToString)
                    'objItem.Mfg_Date = IIf(IsDate(odrOrderItem("MFG_DATE")) = False, "", odrOrderItem("MFG_DATE"))
                    objItem.Price_Per_Pck = IIf(IsNumeric(odrOrderItem("PRICEUNIT")) = False, "", odrOrderItem("PRICEUNIT"))
                    objItem.OrderItem_Price = IIf(IsNumeric(odrOrderItem("PRICE")) = False, "", odrOrderItem("PRICE"))
...
#Region "   Datalayer   "
    ...
    Public Sub AutoGenTag(ByVal pstrOrder_Index As String)
    ...
    Public Function PutAwayWith_Suggest_Location(ByVal pstrOrder_Index As String) As String
    ...
    Public Function GetItemStatus(ByVal pstrItemStatus As String) As DataTable
        Try
            ...
            odaServer.SelectCommand = selectCMD
            ' Add parameters and set values.
            selectCMD.Parameters.Add("@ItemStatus", SqlDbType.NVarChar).Value = pstrItemStatus
    ...
    Public Function GetPackage_Sku(ByVal Sku_Idex As String, ByVal Package_Id As String) As String
        Try
            Dim oConnServer As New SqlClient.SqlConnection(WV_ConnectionString)
            Dim odtServer As New DataTable
            Dim odaServer As New SqlClient.SqlDataAdapter
            ...
            strSQL = "SELECT  P.Package_Index FROM ms_SkuRatio R inner join ms_Package P ON P.Package_Index = R.Package_Index WHERE P.Package_Id ='" & Package_Id & "' AND R.Sku_Index ='" & Sku_Idex & "' AND P.Status_Id <> -1 "
