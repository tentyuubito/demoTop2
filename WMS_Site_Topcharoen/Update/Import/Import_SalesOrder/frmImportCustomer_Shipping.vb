...
Public Class frmImportCustomer_Shipping
    ...
    Private Sub frmImportCustomer_Shipping_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#Region " Validate "
    ...
    Private Function ValidateColumnData(ByVal dgv As DataGridView, ByVal dt As DataTable) As Boolean
        Try
            ...
            ' Values Index
            Dim Default_Customer_Index As String = Nothing
            ...
            For iRow As Integer = 0 To dt.Rows.Count - 1
                Dim clsSelect As New clsCustomer_Shipping()
                ...
                With dt.Rows(iRow)
                        ...
                        If (colArgs.IsRequireData) Then
                            If (dt.Columns.Contains(colArgs.Column_Name_Alias) And Not colArgs.Column_Name_Alias = Nothing) Then
                                ...
                                Select Case colArgs.ColumnType
                                    Case WMS_Site_TopCharoen.ColArgs.ColArgsType.String
                                        IsHasData = (Not .Item(colArgs.Column_Name_Alias).ToString.Trim.Replace("'", "") = Nothing)
                                    Case WMS_Site_TopCharoen.ColArgs.ColArgsType.Date
                                        IsHasData = IsDate(.Item(colArgs.Column_Name_Alias).ToString.Trim.Replace("'", ""))
                                    Case WMS_Site_TopCharoen.ColArgs.ColArgsType.Integer
                                    ...
                                    Case WMS_Site_TopCharoen.ColArgs.ColArgsType.Decimal
                                        IsHasData = IsNumeric(.Item(colArgs.Column_Name_Alias).ToString.Trim.Replace("'", ""))
                                    Case WMS_Site_TopCharoen.ColArgs.ColArgsType.IntegerPositive
                                    ...
                                    Case WMS_Site_TopCharoen.ColArgs.ColArgsType.DecimalPositive
                                    ...
                                    Case WMS_Site_TopCharoen.ColArgs.ColArgsType.IntegerPositiveNotZero
                                        IsHasData = IsNumeric(.Item(colArgs.Column_Name_Alias).ToString.Trim.Replace("'", "")) AndAlso Math.Round(CDec(.Item(colArgs.Column_Name_Alias).ToString.Trim.Replace("'", ""))) / 1 = CDec(.Item(colArgs.Column_Name_Alias).ToString.Trim.Replace("'", "")) AndAlso CDec(.Item(colArgs.Column_Name_Alias).ToString.Trim.Replace("'", "")) > 0
                                    Case WMS_Site_TopCharoen.ColArgs.ColArgsType.DecimalPositiveNotZero
                                        IsHasData = IsNumeric(.Item(colArgs.Column_Name_Alias).ToString.Trim.Replace("'", "")) AndAlso CDec(.Item(colArgs.Column_Name_Alias).ToString.Trim.Replace("'", "")) > 0
                                    Case Else
                    ...
                    ' ตรวจสอบ Customer_Shipping_Id ซ้ำ
                    Dim Customer_Shipping_Index As String = Nothing
                    Col_Alias = Me.getColumn_Name_Alias("Customer_Shipping_Id")
                    If (dt.Columns.Contains(Col_Alias)) Then
                        'Customer_Shipping_Index = clsSelect.SelectIndex(String.Format(" select top 1 Customer_Shipping_Index from ms_Customer_Shipping where status_id<>-1 and Customer_Index='{0}' and Str1='{1}' ", Customer_Index, .Item(Col_Alias).ToString.Trim.Replace("'", "")))
                        Customer_Shipping_Index = clsSelect.SelectIndex(String.Format(" select top 1 Customer_Shipping_Index from ms_Customer_Shipping where status_id<>-1 and Str1='{1}' ", Customer_Index, .Item(Col_Alias).ToString.Trim.Replace("'", "")))
                        If (Not Customer_Shipping_Index = Nothing) Then
                            If (dt.Columns.Contains("Validate")) Then
                                .Item("Validate") = IIf(.Item("Validate") = Nothing, "", .Item("Validate") & ", ") & "มีรหัสผู้รับนี้แล้ว"
                            End If
                            Me.HighListGridView(dgv, iRow, Col_Alias)
                            Continue For
                        End If
...
#Region " Import "
    ...
    Private Function Import_Data(ByVal dt As DataTable) As Boolean
        Try
            ...
            If Not New clsCustomer_Shipping().ImportData(dtTemp, Me.colList) Then
                Return False
            End If
            Return True
        Catch ex As Exception
...
#End Region
    ...
    Private Function getColumnList() As List(Of ColArgs)
        ...
        Try
            ...
            colList.Add(New ColArgs(2, "Customer_Shipping_Desc", "Sold To Name", ColArgs.ColArgsType.String, True, True))
            colList.Add(New ColArgs(3, "Customer_Shipping_Address", "Address", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(4, "SubDistrict_Desc", "ตำบล/แขวง", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(5, "District_Desc", "อำเภอ/เขต", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(6, "Province_Desc", "จังหวัด", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(7, "Postcode", "Postcode", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(8, "Tel", "Tel", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(9, "Fax", "Fax", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(10, "Mobile", "Mobile", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(11, "Email", "Email", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(12, "Remark", "หมายเหตุ", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(13, "Contact_Person1", "ชื่อผู้ติดต่อ 1", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(14, "Contact_Person2", "ชื่อผู้ติดต่อ 2", ColArgs.ColArgsType.String, False, True))
            colList.Add(New ColArgs(15, "Contact_Person3", "ชื่อผู้ติดต่อ 3", ColArgs.ColArgsType.String, False, True))
        Catch ex As Exception
            Throw ex
        End Try
        Return colList
