...
Optional ByVal _Connection As SqlClient.SqlConnection = Nothing, Optional ByVal _myTrans As SqlClient.SqlTransaction = Nothing) As String
        ...
        Try
            ...
            For Each drAutoPicking As DataRow In dtSalesOrderGroup.Rows
                ...
                With drAutoPicking
                    ...
                    Dim Picking_Qty As Decimal = CDec(.Item("Total_Qty").ToString)
                    'Dim xLocationType_Index As String = "2"



                    xWhere.Append(StrWherePicking_Row("FIFO", drAutoPicking, _Connection, _myTrans))
                    '     Dim Location_IndexInSku As String = DBExeQuery_Scalar("select location_index from ms_sku where status_id <> -1 and sku_index =  '" & .Item("Sku_Index").ToString & "'", _Connection, _myTrans)


                    '    strWhere = " and location_index = '" & Location_IndexInSku & "'"

                 
                    objPicking = New WMS_Site_Topcharoen_P2.PICKING(TypePicking, .Item("Sku_Index").ToString, .Item("Package_Index").ToString, Picking_Qty, xWhere.ToString, DocumentType_Index)
                    Pick_ResultAll = objPicking.FIFO_FULL_TAG_SEARCH_PICKING(WMS_Site_Topcharoen_P2.PICKING.enmMode_Pick.OVER_EQUALS, Picking_Qty, False, _Connection, _myTrans, SQLServerCommand)
                    ...
                    xQuery = "1 = 1" 'String.Format("LocationType_Index = '{0}' and Qty_Bal >= {1}", xLocationType_Index, 0)
                    ...
                    If xdrSelect.Length > 0 Then
                        For Each drFIFO As DataRow In xdrSelect
                            ...
                            TypePicking = WMS_Site_Topcharoen_P2.PICKING.enmPicking_Type.FIFO
    ...
    Public Function StrWherePicking_Row(ByVal PickingType As String, ByVal xdrRow As DataRow, Optional ByVal _Connection As SqlClient.SqlConnection = Nothing, Optional ByVal _myTrans As SqlClient.SqlTransaction = Nothing) As String
        Try
            ...
            'End If
            If xdrRow("ERP_Location").ToString.Trim.Length > 0 Then
                _strPicking.Append(String.Format(" AND ERP_Location = '{0}' ", xdrRow("ERP_Location")))
            End If
    ...
    Public Function GetSalesOrderPlan(ByVal arrSalesOrder_Index() As String, ByVal strWhere As String, _
       Optional ByVal _Connection As SqlClient.SqlConnection = Nothing, Optional ByVal _myTrans As SqlClient.SqlTransaction = Nothing) As DataTable
        ...
        Try
            ...
            builder.Append(" SELECT * FROM VIEW_SalesOrderPlan_TopCharoen WHERE 1=1 ")
