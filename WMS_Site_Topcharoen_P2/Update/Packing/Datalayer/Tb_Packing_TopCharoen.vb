...
Public Class Tb_Packing_TopCharoen : Inherits DBType_SQLServer
    ...
    Public Function GetReportInfoWithdrawBox(ByVal pstrReportName As String, ByVal pstrWhere As String) As CrystalDecisions.CrystalReports.Engine.ReportDocument
        ...
        ods.Tables.Add(clsPacking.GetReportData(odt.Rows(0)("View_Name").ToString, "and Barcode_Box = '" & Me.barcodeBox & "'"))
    ...
    Public barcodeBox As String = ""

    Public Function SavePackingWithdraw(ByVal dtTempWithdraw As DataTable, ByVal dtSerial As DataTable, ByVal droppoint As String) As Boolean
        DBconnect()
        Dim Transaction As SqlClient.SqlTransaction = Connection.BeginTransaction()
        Try
            ...
            Dim strSql As String = ""
            If DBExeQuery(" select * from tb_WithdrawBarcode where withdraw_index = '" & dtTempWithdraw.Rows(0).Item("Withdraw_Index") & "'  ", Transaction.Connection, Transaction).Rows.Count > 0 Then
                Me.barcodeBox = DBExeQuery_Scalar(" select barcode_box from tb_WithdrawBarcode where withdraw_index = '" & dtTempWithdraw.Rows(0).Item("Withdraw_Index") & "'  ", Transaction.Connection, Transaction)
                Transaction.Commit()
            ...
            SQL = New System.Text.StringBuilder
            With SQL
                .AppendLine(" INSERT INTO [tb_WithdrawBarcode]  ")
                .AppendLine("       ([Withdraw_Index]  ")
                .AppendLine("       ,[Customer_Shipping_Location_Index]  ")
                .AppendLine("       ,[Barcode_B1]  ")
                .AppendLine("       ,[Barcode_Bag]  ")
                .AppendLine("       ,[Barcode_Box]  ")
                .AppendLine("       ,[Add_Date]  ")
                .AppendLine("       ,[Add_By] )  ")
                .AppendLine(" VALUES   ")
                .AppendLine("       (@Withdraw_Index  ")
            ...
            Dim AutoRunning As New Sy_Running
            Dim BarcodeBox As String = AutoRunning.GetNextRunningBOX("", droppoint)


            For Each dtrow As DataRow In dtSerial.Rows

                With SQLServerCommand.Parameters

                    .Clear()
                    .Add("@Withdraw_Index", SqlDbType.VarChar).Value = dtTempWithdraw.Rows(0).Item("Withdraw_Index")
                    .Add("@Customer_Shipping_Location_Index", SqlDbType.VarChar).Value = dtTempWithdraw.Rows(0).Item("Customer_Shipping_Location_Index")
                    .Add("@Barcode_B1", SqlDbType.VarChar).Value = dtSerial.Rows(i).Item("Serial")
                    .Add("@Barcode_Bag", SqlDbType.VarChar).Value = dtSerial.Rows(i).Item("Barcode_Bag")
                    .Add("@Barcode_Box", SqlDbType.VarChar).Value = BarcodeBox
                    .Add("@Add_Date", SqlDbType.DateTime).Value = Now
    ...
    Public Function GetReportInfoWithdraw(ByVal pstrReportName As String, ByVal pstrWhere As String, ByVal dtTempWithdraw As DataTable, ByVal dtPick As DataTable) As CrystalDecisions.CrystalReports.Engine.ReportDocument
        Dim odt As New DataTable
        Dim oCrystal As New CrystalDecisions.CrystalReports.Engine.ReportDocument
        Dim ods As New DataSet
        Dim clsPacking As New Tb_Packing_TopCharoen
        ...
        '   If odt.Rows(0)("IsVisible") = 1 Then
        oCrystal.Load(WV_Report_Path & odt.Rows(0)("Report_Path").ToString)
        '   End If


        Dim dtReport As New DataTable
        dtReport.Columns.Add("TransportRegion_Desc", GetType(String))
        dtReport.Columns.Add("Route_Id", GetType(String))
        dtReport.Columns.Add("Route_Desc", GetType(String))
        dtReport.Columns.Add("Barcode_BAG", GetType(String))
        dtReport.Columns.Add("Remark", GetType(String))
        dtReport.Columns.Add("Document_No", GetType(String))
        dtReport.Columns.Add("Document_Type", GetType(String))
        dtReport.Columns.Add("ProductType_Id", GetType(String))
        dtReport.Columns.Add("ProductType_Desc", GetType(String))
        dtReport.Columns.Add("Total_Qty", GetType(Decimal))
        dtReport.Columns.Add("Package_Desc", GetType(String))
        dtReport.Columns.Add("Str1", GetType(String))
        dtReport.Columns.Add("Str2", GetType(String))
        dtReport.Columns.Add("Str3", GetType(String))
        dtReport.Columns.Add("Str4", GetType(String))
        dtReport.Columns.Add("Str5", GetType(String))
        dtReport.Columns.Add("Str6", GetType(String))
        dtReport.Columns.Add("Str7", GetType(String))
        dtReport.Columns.Add("Str8", GetType(String))
        dtReport.Columns.Add("Str9", GetType(String))
        dtReport.Columns.Add("Str10", GetType(String))

        dtReport.Columns.Add("pic", GetType(System.Byte()))

        Dim appSet As New Configuration.AppSettingsReader()
        Dim linkImageSig As String = appSet.GetValue("linkImageSig", GetType(String))




        For Each dtrow As DataRow In dtPick.Select("checkPrint = true ") 'dtPick.Rows
            '  If dtrow.Item("checkPrint") Then
            dtrow.Item("checkPrint") = False

            Dim dtrowDetail As DataRow = dtReport.NewRow

            dtrowDetail.Item("TransportRegion_Desc") = dtTempWithdraw.Rows(0).Item("TransportRegion_Desc")
            dtrowDetail.Item("Route_Id") = dtTempWithdraw.Rows(0).Item("Route_ID")
            dtrowDetail.Item("Route_Desc") = dtTempWithdraw.Rows(0).Item("Route_Desc")
            dtrowDetail.Item("Barcode_BAG") = dtrow.Item("Barcode_Bag")
            dtrowDetail.Item("Remark") = dtTempWithdraw.Rows(0).Item("Remark")
            dtrowDetail.Item("Document_No") = dtTempWithdraw.Rows(0).Item("Document_No")
            dtrowDetail.Item("Document_Type") = dtTempWithdraw.Rows(0).Item("Document_Type")
            dtrowDetail.Item("ProductType_Id") = dtTempWithdraw.Rows(0).Item("PackageType_Id")
            dtrowDetail.Item("ProductType_Desc") = dtTempWithdraw.Rows(0).Item("ProductType_Desc")
            dtrowDetail.Item("Total_Qty") = 1 ' dtTempWithdraw.Rows(0).Item("qty")
            dtrowDetail.Item("Package_Desc") = dtTempWithdraw.Rows(0).Item("Package_Desc")
            dtrowDetail.Item("Str1") = "" ' dtTempWithdraw.Rows(0).Item("qty")
            dtrowDetail.Item("Str2") = dtTempWithdraw.Rows(0).Item("Str2")
            dtrowDetail.Item("Str3") = dtTempWithdraw.Rows(0).Item("Str3")
            dtrowDetail.Item("Str4") = dtTempWithdraw.Rows(0).Item("Str4")
            dtrowDetail.Item("Str5") = "" 'dtTempWithdraw.Rows(0).Item("Str5")
            dtrowDetail.Item("Str6") = "" ' dtTempWithdraw.Rows(0).Item("Str6")
            dtrowDetail.Item("Str7") = "" ' dtTempWithdraw.Rows(0).Item("Str7")
            dtrowDetail.Item("Str8") = dtTempWithdraw.Rows(0).Item("Customer_Shipping_Location_Id")
            dtrowDetail.Item("Str9") = WV_UserName
            dtrowDetail.Item("Str10") = Now.ToString("dd-MM-yyyy")
            '   ods.Tables(0).Columns.Add(New DataColumn("pic", GetType(System.Byte())))
            dtrowDetail.Item("Pic") = GetUrlPictoByte(linkImageSig)

            dtReport.Rows.Add(dtrowDetail)
            '   End If
        Next

        ods.Tables.Add(dtReport)

        '    ods.Tables.Add(clsPacking.GetReportData(odt.Rows(0)("View_Name").ToString, pstrWhere))
    ...
    Public Function BmpToByteArray(ByVal data As System.Drawing.Bitmap) As Byte()
        Dim ms As New System.IO.MemoryStream
        Dim r() As Byte

        data.Save(ms, data.RawFormat)
    ...
    Public Function GetReportInfo(ByVal pstrReportName As String, ByVal pstrWhere As String) As CrystalDecisions.CrystalReports.Engine.ReportDocument
        ...
        For i As Integer = 0 To odt.Rows.Count - 1
            ...
            If pstrReportName = "rptTransferDocRange" Then
                ods.Tables(0).Columns.Add("Pic", GetType(Byte()))
                ods.Tables(0).Rows(0).Item("Pic") = GetUrlPictoByte(linkImageSig)
    ...
    Public Function GetUrlPictoByte(ByVal urlPic As String) As Byte()
        Try

            Dim wc As New Net.WebClient()
            Dim bytes As Byte() = wc.DownloadData(urlPic)
    ...
    Public Function checkBarcodeB1ByWithdraw(ByVal withdraw_index As String) As Integer
        Try

            Dim strSql As String = ""
            strSql = " select count(*) from tb_withdrawBarcode where withdraw_index = '" & withdraw_index & "' group by withdraw_index "
